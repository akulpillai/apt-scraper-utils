...
source_file_to_read_packages_from = "/workdisk/akul/gcc_support/debian_packages/Sources"
mirror_url = "http://mirror.math.ucdavis.edu/ubuntu/"
local_download_folder_for_sources = "/workdisk/akul/gcc_support/debian_packages/downloads/"
extracted_tar_sources = '/workdisk/akul/gcc_support/debian_packages/extracted_tar_sources/'
afl_fuzzing_sources = '/workdisk/akul/gcc_support/debian_packages/afl_sources/'
ALL_OPTIONS_PATH="/workdisk/akul/RandoLLvM/gcc_all_options.txt"
CURR_OPTIONS_PATH="/workdisk/akul/test_dir/empty.txt" #point to an empty file
    ...
    for File in files:
        if "orig" in str(File):
            ...
            if( os.path.exists(configure_path) == True ):
                ...
                cmd = "(" + "cd " + local_download_folder_for_sources + "/" + directory_name + " && " + "export COPA_COMPILER=gcc; export COPA_CXX_COMPILER=g++; export COPA_ALL_OPTIMIZATION_FLAGS_FILE=" + ALL_OPTIONS_PATH + "; export COPA_CURR_OPTIMIZATION_FLAGS_FILE=" + CURR_OPTIONS_PATH + ";" + " && " + "CC=wllvmcopa CXX=wllvmcopa++ ./configure" +  " && " + "make" + " && " + "make install DESTDIR=" + extracted_tar_sources + "/" + directory_name  + ")"
                subprocess.call(cmd, shell=True)
